...
/* ---------- Data ---------- */
const ITEMS: Record<string, { id: string; title: string; price: number; emoji: string }[]> = {
    laundry: [
...
export default function SelectItems() {
    ...
    const { items: cartItems, addItem, removeItem, setQty, clear, getQty, total } = useCart();
        ...
        <View style={[styles.container, { backgroundColor: theme.background }]}>
            ...
            <View style={[styles.header, { backgroundColor: theme.card, borderBottomColor: theme.border }]}>
                ...
                <View style={styles.headerCenter}>
                    <Text style={[styles.headerTitle, { color: theme.text }]}>Add Items</Text>
                    {orderId && <Text style={[styles.headerSubtitle, { color: theme.subText }]}>Order #{orderId}</Text>}
                </View>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>
                    {SERVICES.map((service) => {
                        ...
                        return (
                            ...
                            >
                                <Ionicons name={service.icon as any} size={18} color={isActive ? "#fff" : theme.text} />
                                <Text style={[styles.tabText, { color: isActive ? "#fff" : theme.text }]}>{service.label}</Text>
                            </TouchableOpacity>
                        );
                    })}
                </ScrollView>
            ...
            <View style={styles.content}>
                ...
                >
                    <Text style={[styles.sectionTitle, { color: theme.text }]}>Available Items</Text>
                    ...
                    <View style={styles.smallRowList}>
                        ...
                        {availableItems.map((item) => (
                            <View
                                key={item.id}
                                style={[styles.smallRowCard, { backgroundColor: theme.card, borderColor: theme.border }]}
                            >
                                ...
                                <TouchableOpacity
                                    onPress={() =>
                                        addItem({ id: item.id, title: item.title, price: item.price }, 1)
                                    }
                                    style={[styles.smallAddBtn, { backgroundColor: theme.primary }]}
                                >
                    ...
                    {cartItemsArray.length > 0 && (
                        <View style={[styles.checkoutCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                            <Text style={[styles.checkoutHeading, { color: theme.text }]}>Items in Order</Text>

                            {cartItemsArray.map((it: any) => (
                                <View key={it.id} style={styles.checkoutItemRow}>
                                    <Text style={[styles.checkoutItemEmoji, { color: theme.text }]}>{findEmoji(it.id)}</Text>

                                    <View style={styles.checkoutItemInfo}>
                                        <Text style={[styles.checkoutItemTitle, { color: theme.text }]} numberOfLines={1}>
                                            {it.title}
                                        </Text>
                                        <Text style={[styles.checkoutItemMeta, { color: theme.subText }]}>
                                    ...
                                    <View style={styles.checkoutQtyControls}>
                                        ...
                                        <Text style={[styles.qtyText, { color: theme.text }]}>{it.qty}</Text>

                                        <TouchableOpacity
                                            onPress={() => setQty(it.id, it.qty + 1)}
                                            style={[styles.qtyBtnPrimary, { backgroundColor: theme.primary }]}
                                        >
                                            <Ionicons name="add" size={16} color="#fff" />
                                        </TouchableOpacity>

                                        <TouchableOpacity onPress={() => deleteItem(it.id)} style={styles.deleteBtn}>
                ...
                <View style={[styles.summaryPanel, { backgroundColor: theme.card, borderColor: theme.border }]}>
                    ...
                    <View style={[styles.summaryTopRow, styles.totalRow]}>
                        ...
                        <Text style={[styles.totalValue, { color: theme.primary }]}>₹{payable}</Text>
                    </View>
                    ...
                    <View style={styles.checkoutRow}>
                        ...
                        <TouchableOpacity onPress={() => clear()} style={[styles.clearBtn, { borderColor: theme.border }]}>
                            <Text style={[styles.clearBtnText, { color: theme.subText }]}>Clear</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                        ...
                        <View style={[styles.modalCard, { backgroundColor: theme.card }]}>
                            ...
                            {cameraPermission?.granted ? (
                                ...
                                <View style={[styles.camera, { alignItems: "center", justifyContent: "center" }]}>
                                    <Text style={{ color: theme.subText }}>Camera permission required</Text>
                                </View>
                            )}
...
const styles = StyleSheet.create({
    ...
    topGrid: { flexDirection: "row", justifyContent: "space-between", marginBottom: 10 },
    featureCard: {
        flex: 1,
        flexDirection: "row",
        alignItems: "center",
    ...
    qtyText: { minWidth: 28, textAlign: "center", fontSize: 15, fontWeight: "700" },
    ...
    summaryTopRow: { flexDirection: "row", justifyContent: "space-between", marginBottom: 8 },
    ...
    checkoutBtn: { flex: 1, paddingVertical: 12, borderRadius: 10, alignItems: "center", justifyContent: "center" },
    checkoutBtnText: { color: "#fff", fontSize: 16, fontWeight: "700" },
    clearBtn: { paddingVertical: 12, paddingHorizontal: 12, borderRadius: 10, borderWidth: 1, justifyContent: "center" },

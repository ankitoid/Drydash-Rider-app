...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const ordered = [...SAMPLE_ORDERS].sort((a, b) => b.priority - a.priority);
      Animated.stagger(
        80,
        ordered.map((o) =>
          Animated.parallel([
  ...
  if (loading) {
    return (
      <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={{ paddingTop: 18, paddingBottom: 120 }}>
        ...
        <View style={{ paddingHorizontal: 16 }}>
          <Text style={[styles.sectionTitleSmall, { color: theme.subText, marginTop: 18 }]}>Upcoming</Text>
          <Skeleton height={H_CARD} radius={14} style={{ marginTop: 12 }} />
          <Skeleton height={H_CARD} radius={14} style={{ marginTop: 12 }} />
          <Text style={[styles.sectionTitleSmall, { color: theme.subText, marginTop: 18 }]}>History</Text>
  ...
  const upcomingPickups = orders.filter((o) => o.kind === "pickup" && o.priority >= 7).sort((a, b) => b.priority - a.priority);
  const upcomingDeliveries = orders.filter((o) => o.kind === "delivery" && o.priority >= 7).sort((a, b) => b.priority - a.priority);
    ...
    <FlatList
      ...
      ListHeaderComponent={
        <>
          ...
          <SectionHeader title="Priority Pickups" subtitle={upcomingPickups.length ? `${upcomingPickups.length} waiting` : "None"} />
          {upcomingPickups.length > 0 ? (
            <FlatList
              ...
              renderItem={({ item }) => (
                <Animated.View style={{ opacity: itemOpacities[item.id] ?? 1, transform: [{ translateY: itemTranslates[item.id] ?? 0 }] }}>
                  <UpcomingCard order={item} theme={theme} onPress={() => router.push(`/(rider)/order/pickup/${item.id}`)} />
                </Animated.View>
              )}
              keyExtractor={(i) => i.id}
            />
          ...
          {/* Upcoming deliveries */}
          <SectionHeader title="Priority Deliveries" subtitle={upcomingDeliveries.length ? `${upcomingDeliveries.length} waiting` : "None"} />
          {upcomingDeliveries.length > 0 ? (
            <FlatList
              ...
              renderItem={({ item }) => (
                <Animated.View style={{ opacity: itemOpacities[item.id] ?? 1, transform: [{ translateY: itemTranslates[item.id] ?? 0 }] }}>
                  <UpcomingCard order={item} theme={theme} onPress={() => router.push(`/(rider)/order/delivered/${item.id}`)} />
                </Animated.View>
              )}
              keyExtractor={(i) => i.id}
            />
          ...
          <Animated.View style={[styles.topRow, { opacity: kpi.opacity, transform: [{ translateY: kpi.translateY }] }]}>
          ...
          {/* KPI Cards */}
          <Animated.View style={[styles.kpiContainer, { opacity: kpi.opacity, transform: [{ translateY: kpi.translateY }] }]}>
...
function SectionHeader({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <View style={{ paddingHorizontal: 16, marginTop: 18, marginBottom: 6, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
      <Text style={{ fontSize: 16, fontWeight: "800" }}>{title}</Text>
      {subtitle ? <Text style={{ fontSize: 13, color: "#94A3B8", fontWeight: "700" }}>{subtitle}</Text> : null}
    </View>
  );
}

function UpcomingCard({ order, theme, onPress }: { order: Order; theme: any; onPress: () => void }) {
  return (
    <TouchableOpacity activeOpacity={0.9} onPress={onPress} style={[styles.upcomingCard, { width: H_CARD, backgroundColor: theme.card, borderColor: theme.border }]}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
        <View>
          <Text style={{ fontWeight: "900", fontSize: 16, color: theme.text }}>{order.kind === "pickup" ? "Pickup" : "Delivery"}</Text>
          <Text style={{ marginTop: 6, color: theme.subText }}>{order.address ?? order.to}</Text>
          <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
            <View style={styles.pillMini}>
              <Text style={{ fontWeight: "800", color: "#065F46" }}>{order.eta ?? order.time}</Text>
            </View>
            <View style={styles.pillMiniOutline}>
              <Text style={{ fontWeight: "800", color: theme.primary }}>{order.distance ?? ""}</Text>
            </View>
          </View>
        </View>

        <View style={{ alignItems: "flex-end" }}>
          <Text style={{ fontWeight: "900", color: theme.primary }}>{order.price}</Text>
          <View style={{ marginTop: 12, backgroundColor: "#F0FDF4", paddingHorizontal: 8, paddingVertical: 6, borderRadius: 999 }}>
            <Text style={{ color: "#166534", fontWeight: "800" }}>{order.status}</Text>
          </View>
        </View>
      </View>
    </TouchableOpacity>
...
function EmptyMini({ label }: { label: string }) {
  return (
    <View style={{ paddingHorizontal: 16 }}>
      <View style={{ padding: 12, borderRadius: 12, backgroundColor: "#0F172A10", marginBottom: 6 }}>
...
function EmptyList({ theme }: { theme: any }) {
  return (
    <View style={{ padding: 16 }}>
      <Text style={{ color: theme.subText }}>No orders yet — pull to refresh</Text>
    </View>
  );
}
...
function AnimatedOrderRow({ order, theme, translateY, opacity }: { order: Order; theme: any; translateY: Animated.Value; opacity: Animated.Value }) {
  ...
  const onPress = () => {
    const route = order.kind === "pickup" ? `/(rider)/order/pickup/${order.id}` : `/(rider)/order/delivered/${order.id}`;
    router.push(route);
  };

  const statusColor = order.status === "Delivered" ? "#16A34A" : order.status === "Active" ? "#06B6D4" : "#F59E0B";

  return (
    <Animated.View style={{ opacity, transform: [{ translateY }] }}>
      <Animated.View style={[styles.orderCard, { backgroundColor: theme.card, borderColor: theme.border, transform: [{ scale }] }]}>
        <TouchableOpacity activeOpacity={0.95} onPressIn={onPressIn} onPressOut={onPressOut} onPress={onPress} style={{ flexDirection: "row", alignItems: "center" }}>
          <View style={styles.orderLeft}>
            <View style={[styles.orderIconWrap, { backgroundColor: theme.background }]}>
              <Ionicons name={order.kind === "pickup" ? "locate" : "cube"} size={18} color={theme.primary} />
            </View>
          </View>

          <View style={styles.orderCenter}>
            <Text style={[styles.orderId, { color: theme.text }]}>{order.id}</Text>
            <Text style={[styles.orderTime, { color: theme.subText }]}>{order.time} · {order.eta ?? ""}</Text>
            <Text style={[styles.orderMeta, { color: theme.subText }]} numberOfLines={1}>
              {order.kind === "pickup" ? order.address : `${order.to} • ${order.from ?? ""}`}
            </Text>
          </View>

          <View style={styles.orderRight}>
            <Text style={[styles.orderPrice, { color: theme.primary }]}>{order.price}</Text>
